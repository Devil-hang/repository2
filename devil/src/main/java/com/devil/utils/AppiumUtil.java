...
public class AppiumUtil {
	...
	//创建driver
	public AppiumDriver<WebElement> getDriver(String platformName, String url, DesiredCapabilities capabilities){
		logger.info("准备初始化driver...");
		try {
...
//			break;
	...
	/**滑动*/
	public void swipe(int beginX,int beginY,int endX,int endY, int seconds){
		Duration duration = Duration.ofSeconds(seconds);
		TouchAction tAction=new TouchAction(driver);
		try{
		tAction.press(beginX,beginY).waitAction(duration).moveTo(endX,endY).release().perform();
		logger.info("滑动成功");
		}catch(Exception e){
			e.printStackTrace();
			logger.error("滑动成功", e);
	...
	public void zoomAndPinch(int beginX,int beginY,int endX,int endY){
	   ...
	   int scrWidth = driver.manage().window().getSize().getWidth();
		...
		tAction0.press(scrWidth/2,scrHeight/2).waitAction().moveTo(beginX,beginY).release();
		tAction1.press(scrWidth/2,scrHeight/2+40).waitAction().moveTo(endX,endY).release();
	...
	/**app置于后台运行*/
	public void runBackgound(Duration runTimes){
	...
	/**使用adb输入*/
	public void adbInput(String string) {
		Runtime runtime = Runtime.getRuntime();
		try {
			runtime.exec("cmd /c adb shell input text " + string);
			logger.info("使用adb命令输入:"+string);
		}catch (Exception e) {
			logger.error("adb 输入失败.", e);
		}
	}
	/**获取源码*/
	...
	public void swipeControl(By by, Heading heading) {
		...
		Dimension dimension = driver.findElement(by).getSize();
		//获取控件坐标轴差
		...
		switch(heading) {
		case UP:
			swipe(centreX, centreY, centreX, centreY-y/4, 1);
			break;
		case DOWN:
			swipe(centreX, centreY, centreX, centreY+y/4, 1);
			break;
